...
from function.feature_functions import *
...
def command():
    while True:
        ...
        # Check wakeup keywords
        if text_lower in [keyword.strip() for keyword in wakeup_key_word]:
            welcome()
            continue
...
def process_command(text):
    ...
    # Check if 'jarvis' is present
    if "jarvis" in text:
        # Remove all occurrences of 'jarvis' and clean up spaces
        cleaned_text = re.sub(r"\bjarvis\b", "", text).strip()

        if not cleaned_text:  # If only jarvis was said
            welcome()
            return

        text = cleaned_text

    # Check if the first word is an open/close command
    first_word = text.split()[0] if text else ""

    if first_word in open_input:
        # Remove the first word (which is the open command)
