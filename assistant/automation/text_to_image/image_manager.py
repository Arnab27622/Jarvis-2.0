...
class ImageManager:
    ...
    def _run_stability(self, prompt: str) -> bool:
        print("[ImageManager] Attempting generation with Stability AI...")
        res = gen_stability(prompt)
        # Returns False if it fails, otherwise implicitly None/True
        return res is not False

    def _run_cloudflare(self, prompt: str) -> bool:
        print("[ImageManager] Attempting generation with Cloudflare Flux...")
        success, _ = gen_cloudflare(prompt)
        return success

    def _run_pollinations(self, prompt: str) -> bool:
        print("[ImageManager] Attempting generation with Pollinations Flux...")
        res = gen_pollinations(prompt)
        # Returns False if it fails, otherwise implicitly None/True
        return res is not False

    def generate_image(self, prompt: str, preferred_engine: Optional[str] = None) -> bool:
        """
...
def generate_image(prompt: str, engine: Optional[str] = None) -> bool:
